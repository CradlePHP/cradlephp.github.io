...
[2.1. Creating a Schema](/docs/admin/create-schema.html).
...
Go ahead and update the **Article** and you should be redirected back to the
**Schema Search** page with the following relations below now set.
...
is the title shows the title of the article then points to the number of
comments.
...
The next thing to do is create a comment. Click the green `Add Comment` button
on the top right. When we create a **Comment** this way, it will create it and
automatically link the new comment with the **Article**.
...
When a **Comment** is created linking to an **Article**, a new pink action row
will show allowing you to unlink the comment if you so wish. Right now, go ahead
and click it.

###### Figure 2.3.3.D. Unlink a Comment
![Unlink a Comment](/docs/admin/assets/2.3.3.D.png)

This will unlink the **Comment** from the **Article**, but the **Comment** will
still exist so don't worry. If we want to re-link the **Comment** to the
**Article** once again, click the yellow `Link Comment` button on the top right.

###### Figure 2.3.3.E. Link a Comment
![Link a Comment](/docs/admin/assets/2.3.3.E.png)

This will redirect you to a simple form where you can link a **Comment**. Since
we made `comment_detail` searchable just enter any word that exists in the
**Comment** and the suggestion format of the **Comment**
...
**Comment** this way, it will create it and automatically link the new comment
...
our existing **Article**. We also made a **Comment** relate to itself. Based on
just these constructs, we can create complicated systems without needing to open
a code editor. This also helps bridge the gap between client and code business
rules much easier. In the next section,
[2.4. Adding Menu Items](/docs/admin/adding-menu-items.html) we are going to
